...
int main(){ 
	//Declaring 4 arrays with the CV class type, each of arrays have the same veriables "salary","position" and "name", The veriable "position" has the same value for the all objects.
	CV array[5]; 
	CV array1[5]; 
	CV array2[5]; 
	CV array3[5];
	...
	string Pos=pos(); //Declaring the variable Pos, which inputs the user in pos() function. 
        int MinEx=minex(); //Declaring the variable MinEx and MaxSal, which inputs user in minex() and maxsal() function.
        int MaxSal=maxsal();

        cout << "There are our employers, with your criteries.\n";
	Print(array, array1, array2, array3, Pos, MinEx, MaxSal);//Printing appropriate "name" veriables        
}
